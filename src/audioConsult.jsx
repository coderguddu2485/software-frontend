...
const AudioConsult = ({
  ...
  const handleSearch = async (value) => {
    ...
    if (value.length > 1) {
      try {
        // Appending ?type=audio to only fetch audio-enabled doctors
        const response = await fetch(
          `http://localhost:3000/doctors/${value}?type=audio`,
        );
    ...
    <div className="bg-gray-50 min-h-screen font-sans">
      ...
      <div className="max-w-6xl mx-auto py-12 px-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-8">
          {doctors.length > 0
            ? `Audio Specialists for "${searchTerm}"`
            ...
            >
              ...
              <div className="w-full border-t pt-4 mt-auto flex justify-between items-center">
                <div className="text-left">
                  <p className="text-xs text-gray-500 uppercase">Audio Fee</p>
